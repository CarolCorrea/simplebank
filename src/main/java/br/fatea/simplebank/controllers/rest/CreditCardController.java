...
public class CreditCardController {
	...
	public ResponseEntity<HTTPResource> save(@RequestBody @Valid CreditCardResource resource, BindingResult bindingResult){ 
		if(bindingResult.hasErrors()){ 
			...
			resourceResponse.add(linkTo(methodOn(this.getClass()).findByNumber(creditCard.getNumber())).withSelfRel()); 
			resourceResponse.add(linkTo(methodOn(this.getClass()).update(resource, creditCard.getNumber(), bindingResult)) .withRel("update"));
			resourceResponse.add(linkTo(methodOn(this.getClass()).delete(creditCard.getNumber())) .withRel("delete"));
	...
	public ResponseEntity<HTTPResource> update( @RequestBody @Valid CreditCardResource resource, @PathVariable String number, BindingResult bindingResult) { 
		if (bindingResult.hasErrors()) { 
			return new ResponseEntity<HTTPResource>(HttpStatus.BAD_REQUEST); 
		} else { 
